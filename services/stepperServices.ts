...
export const createFacility = async (token: string, IdToken: string, payload: any) => {
  ...
  if(existingFacility?.facilityId){
    const result = await apiRequest(`create-facility?id=${existingFacility?.facilityId}`, "POST", payload, token, IdToken);
    if(result.success){
      return result.data;
    }
...
export const getFacility = async (token: string, IdToken: string) => {
  try {
    const result = await apiRequest("get-user-facility", "GET", undefined, token, IdToken);
    ...
    return {
      ...
      electricityConsumption: {
        ...
        electricityIndependentDKK: facilityData.electricityConsumption?.electricityIndependentDKK || "",
        electricityDependentDKK: facilityData.electricityConsumption?.electricityDependentDKK || "",
      },
    };
